...
* 事务的ACID特性
  ...
  * 一致性：事务开始前和结束后，数据库的完整性约束没有被破坏，都是合法的数据状态。比如A向B转账，不可能A扣了钱，B却没收到。 ![image](https://s2.ax1x.com/2020/02/13/1Ltmq0.png)
  * 隔离性：并发执行的事务不会相互影响，其对数据库的影响和它们串行执行时一样。 ![image](https://s2.ax1x.com/2020/02/13/1LYhvR.png) ![image](https://s2.ax1x.com/2020/02/13/1LY7VK.png)
  * 持久性：事务一旦提交，其对数据库的更新就是持久的。任何事务或系统故障都不会导致数据丢失。 ![image](https://s2.ax1x.com/2020/02/13/1LYW8J.png)
...
* 如何避免数据库一致性被破坏
  ...
  * 日志恢复技术：保证了事务的原子性，使数据库的一致性不会因事务或系统故障被破坏。同时使已提交的对数据库的修改不会因系统崩溃而丢失，保证了事务的持久性。 ![image](https://s2.ax1x.com/2020/02/13/1LklXn.png)
